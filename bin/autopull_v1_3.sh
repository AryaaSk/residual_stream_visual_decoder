...
# Polls the remote every 5 min, pulls any new artefacts under findings/v1_3/
# and artefacts/v1_3/. Auto-terminates after 12 hours or when SUMMARY.json
# is present locally (whichever comes first).

set +e
cd "$(dirname "$0")/.."

LOG=runs/autopull_v1_3.log
mkdir -p runs
...
while [[ $(date +%s) -lt "$DEADLINE" ]]; do
    if [[ -f "findings/v1_3/SUMMARY.json" ]]; then
        log "SUMMARY.json present locally — pipeline finished, final pull"
        rsync -avz -e "ssh -i $H200_KEY -o StrictHostKeyChecking=no" \
            "$H200_USER@$H200_HOST:$REMOTE_DIR/findings/v1_3/" findings/v1_3/ 2>&1 | tail -3 | tee -a "$LOG"
        rsync -avz -e "ssh -i $H200_KEY -o StrictHostKeyChecking=no" \
            "$H200_USER@$H200_HOST:$REMOTE_DIR/artefacts/v1_3/" artefacts/v1_3/ 2>&1 | tail -3 | tee -a "$LOG"
        rsync -avz -e "ssh -i $H200_KEY -o StrictHostKeyChecking=no" \
        ...
        /usr/bin/env python3 code/eval/fill_writeup.py \
            --summary findings/v1_3/SUMMARY.json \
            --writeup WRITEUP.md 2>&1 | tee -a "$LOG"
        log "done."
        exit 0
    fi
    rsync -avz --partial -e "ssh -i $H200_KEY -o StrictHostKeyChecking=no" \
        "$H200_USER@$H200_HOST:$REMOTE_DIR/findings/v1_3/" findings/v1_3/ 2>/dev/null | tail -3 >> "$LOG"
    rsync -avz --partial -e "ssh -i $H200_KEY -o StrictHostKeyChecking=no" \
        "$H200_USER@$H200_HOST:$REMOTE_DIR/artefacts/v1_3/" artefacts/v1_3/ 2>/dev/null | tail -3 >> "$LOG"
    rsync -avz --partial -e "ssh -i $H200_KEY -o StrictHostKeyChecking=no" \
