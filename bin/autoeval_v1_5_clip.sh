...
LOG=runs/autoeval_v1_5_clip.log
...
run_clip_on_ckpt() {
    ...
    local TAG=$3
    local GPU=$4
    local OUT="findings/v1_5/clip_L${LAYER}_${TAG}"
    if [[ -d "$OUT" ]]; then
        log "skip $OUT (exists)"
        return
    fi
...
while true; do
    L12_DONE=$(grep -c "^\[s15\] DONE → " runs/s15_L12.log 2>/dev/null)
    L24_DONE=$(grep -c "^\[s15\] DONE → " runs/s15_L24.log 2>/dev/null)

    for L in 12 24; do
        if [[ "$L" == "12" ]]; then EVAL_GPU=2; else EVAL_GPU=3; fi
        for CKPT in $(ls -1d checkpoints/v1_5/L${L}/step_* 2>/dev/null); do
    ...
    for L in 12 24; do
        ...
        if [[ "$DONE_FLAG" -ge 1 && "${SEEN[L${L}_final]:-0}" == "0" ]]; then
            SEEN[L${L}_final]=1
            FINAL_DIR="checkpoints/v1_5/L${L}/final"
            if [[ -d "$FINAL_DIR" ]]; then
...
heading "CLIP AUTOEVAL COMPLETE. Outputs under findings/v1_5/clip_L*_*"
